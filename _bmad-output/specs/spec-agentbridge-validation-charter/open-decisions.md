...
> **Исторический регистр.** Existential Gate 1 superseded решением Native Architecture-First от 2026-09-15. Статусы ниже сохраняются для traceability и не блокируют AA-1. Текущие решения находятся в `../spec-agentbridge-architecture-assurance/open-decisions.md`.

Исторически, пока хотя бы одна строка была открыта, прежний Candidate experiment имел статус **No Start**. Это правило по-прежнему запрещает запуск старого эксперимента, но не блокирует AA-1 нового Architecture Assurance Charter. Рекомендации ниже сохранены как история решений.
